...
export default function drag(event) {
    if (session.SHAPE_STATE == constants.SHAPE_STATES.DRAG_STARTING) {
        /**
         * @type {SVGGElement} g
         */
        ...
        session.SHAPE_STATE = constants.SHAPE_STATES.DRAGGING;
        // Trigger event
        Events.shape.drag.started();
    }

    if (session.SHAPE_STATE == constants.SHAPE_STATES.DRAGGING && session.SAVED_MOUSE_POS) {
