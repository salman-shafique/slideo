...
const entryAnalyze = (nextEvent) => {
    ...
    try {
        ...
        switch (nextEvent) {
            case "RM_FORMAT":
                const entryRemoveFormatBtn = document.getElementById("entry_remove_format");
                entryRemoveFormatBtn.click();
                entryAnalyze("INNER_TEXT");
                return;
            case "INNER_TEXT":
                session.EDITOR.innerHTML = session.EDITOR.innerText;
                entryAnalyze("ERROR");
                return;
            case "ERROR":
                toastr.error("We couldn't analyze your text. Please try simpler.", "Oops!");
                return;
            default:
                entryAnalyze("RM_FORMAT");
                return;
        }
    }
}
