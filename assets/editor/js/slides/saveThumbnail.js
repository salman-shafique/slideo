...
const saveThumbnail = (event) => {
    const aSlide = session.PRESENTATION.slides[0];
    if(event.type != "saveChange.inited" && session.CURRENT_SLIDE != aSlide.slideId) return

    const slide_ = slide(aSlide.slideId);
    const slideData = slide_.slideData();
    const imgShapes = slideData.shapes.filter(shape_ => {
        const Shape = shape(aSlide.slideId, shape_.data.shape_id);
            const g = Shape.el();
...
Events.listen("saveChange.content", (event) => {
    if(session.CURRENT_SLIDE != session.PRESENTATION.slides[0].slideId) return
