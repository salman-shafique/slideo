...
export default function IconEditorContent() {
  ...
  useEffect(() => {
    Events.listen("presentation.inited", (e) => {
      setTimeout(async () => {
        ...
        const keywords = new Set(
          icons.map((icon) => icon.data.keyword?.toLowerCase()).filter((e) => e)
        );
    ...
    Events.listen("popup.icon.open", (e) => {
      const shapes = session.PRESENTATION.slides.filter(
        (slide) => slide.slideId === session.CURRENT_SLIDE
      )[0].shapes;
      const selectedShape = session.SELECTED_ELEMENTS[0].shapeId;

      const keyword = shapes.filter(
        (shape) => shape.data.shape_id == selectedShape
      )[0].data.keyword;
      hook.setKeywords(
        hook.keywords.concat({ keyword: keyword.toLowerCase(), active: false })
      );
      // hook.fetchNewData(keyword.toLowerCase())

      if (
        hook.keywords.filter(
          (oldKeyword) => oldKeyword.keyword === keyword.toLowerCase()
        ).length
      ) {
      ...
      hook.keywords.map((k) =>
        k.keyword !== keyword ? (k.active = false) : null
      );
      // hook.fetchNewData(keyword.toLowerCase())
      hook.setKeywords(
        (hook.keywords = [
          { keyword: keyword.toLowerCase(), active: true },
          ...hook.keywords,
        ])
      );
    });
  ...
  return (
    <>
      <SearchSection
        ...
        setKeywords={hook.setKeywords}
        fetchNewData={hook.fetchNewData}
        type="icon"
      >
        <div className="col-9 m-0 d-flex align-items-center my-white-text">
          Icon Color
        </div>
